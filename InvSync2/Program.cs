...
static class dataBaseServer
{
    ...
    static string ExePath = new FileInfo(System.Diagnostics.Process.GetCurrentProcess().MainModule.FileName).DirectoryName;
    ...
    public static void Main(string[] args)
    {
        ...
        InvSync.Listen(new IPEndPoint(IPAddress.IPv6Any, 7342));

        List<string> ips = new List<string>();

        if (!File.Exists(ExePath + "/settings.yaml"))
        {
            Logger.LogWarn($"Couldn't find settings.yaml\nCreating default one in {ExePath}\nCreating default one:\n{defaultSettings}\n");

            File.WriteAllText($"{ExePath}/settings.yaml", defaultSettings);
        }

        {
            Logger.Log("Reading Settings.yaml...");
                ...
                if (!lines[i].StartsWith("#"))
                {
                    ...
                    if (lines[i].StartsWith("IPs:"))
                    {
                        ...
                        while (lines[i].Trim().StartsWith("-"))
                        {
                            ...
                            {
                                Logger.Log("Unknown argument: \"" + lines[i] + "\"");
                            }
                    ...
                    else if (lines[i].StartsWith("Pl_To: "))
                    {
                        ...
                        {
                            preloginTo = val;
                            Logger.Log("Prelogin time out: " + val);
                        }
                        else
                            Logger.Log("Unknown argument: \"" + lines[i] + "\"");
                    }
        ...
        Task.Run(() =>
        {
            ...
            while (running == 1)
            {
                ...
                try
                {
                    Logger.TitleWrite($"{ playerCount + " Online   " + Servers.Count + " Server   " + prelogin.Count + " Prelogin " + ps.ToString("F2") + "/s"}");
                }
                catch (Exception)
                {
                }
        ...
        while (true)
        {
            ...
            {
                if (lastLock != "")
                    Logger.Log("Waiting for: " + lastLock);
                ...
                lock (Servers)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Logger.Log("There are currently no server running");
                        }
                    }
                    catch (Exception e)
                    {
            ...
            else if (s == "ext")
            {
                if (extIp.Count > 0)
                {
                    Logger.Log("Unauthorized Ips connections requests:");
                    string[] ipss = extIp.ToArray();
                    foreach (string ss in ipss)
                        Logger.Log(ss);
                }
                else
                    Logger.Log("There are no unauthorized ips that have tried to connect");

            }
            else
            {
                Logger.Log("Command list: ");
                Logger.Log("stop: stop the database correctly");
                Logger.Log("list: list all registered servers");
                Logger.Log("ext: list all not whitelisted ips that tryied to connect");
            }
    ...
    {
        if (!s.Connected)
            Logger.LogError("Socket was not connected");
        ...
        try
        {
            ...
            {
                Logger.LogError("Couldn't read packet size");
                return;
            }
            ...
            {
                Logger.LogError("Invalid packet size: " + size);
                return;
            }
            ...
            {
                Logger.LogError($"Couldn't read {size} bytes");
                return;
            }
            ...
            switch (packetid)
            {
                case 0://demande du fichier
                    ...
                    Logger.Log("FileRequest of " + name + ".dat");
                ...
                rst:
                    ...
                    {
                        Logger.LogError($"Error in file request of {name}\n{e}");
                    }
                ...
                case 1://ecrire le fichier
                    ...
                    Logger.Log("SaveRequest of " + name + ".dat");
                ...
                rst2:
                    ...
                    try
                    {
                        ...
                        if (filesize > 0)
                        {
                            ...
                            //Logger.Log("SaveRequest Writed " + name + ".dat -> " + value.ToString("X") + " " + file.Length + " bytes");
                        }
                        else
                        {
                            SendToSocket(s, BitConverter.GetBytes(1));
                    ...
                    {
                        Logger.LogError($"Error in save request of {name}\n{e}");
                    }
                ...
                case 2://add a server
                    ...
                    {
                        Logger.LogError($"Error adding server: {name}\n{e}");
                    }
                ...
                case 3://login
                    ...
                    try
                    {
                        if (Servers.TryGetValue(name, out List<string> serv))
                        {
                            ...
                            lock (prelogin)
                            {
                                        ...
                                        foreach (List<string> servs in Servers.Values)
                                        {
                                            lock (servs)
                                            {
                                                if (servs.Contains(name2))
                                                {
                                                    ...
                                                    Logger.LogWarn($"Player {name2} is allready connected");
                                ...
                                lock (serv)
                                {
                                    Logger.Log("login " + name2 + " on: " + name);
                                    serv.Add(name2);
                    ...
                    {
                        Logger.LogError($"Error in login of: {name}\n{e}");
                    }
                ...
                case 4://disconnect
                    ...
                    try
                    {
                        if (Servers.TryGetValue(name, out List<string> serv2))
                        {
                            ...
                            lock (serv2)
                            {
                                ...
                                {
                                    Logger.Log("Disconecting " + name2 + " from: " + name);
                                    serv2.Remove(name2);
                                }
                                else
                                {
                                    Logger.LogWarn($"Player {name2} is not in {name} (Disconnect Request)");
                                }
                    ...
                    {
                        Logger.LogError($"Error in disconnect of: {name}\n{e}");
                    }
                ...
                case 5://player count
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Logger.LogWarn("Server: " + name + " Doesn't exist");
                    ...
                    {
                        Logger.LogError($"Error in playercount of: {name}\n{e}");
                    }
                    ...
                    try
                    {
                        lock (prelogin)
                        {
                            if (prelogin.ContainsKey(name) && prelogin.TryGetValue(name, out DateTime t0))
                            {
                                if ((DateTime.Now - t0).TotalMilliseconds < preloginTo)
                                {
                                    ...
                                    Logger.LogWarn($"Player {name} is allready preloged");
                                    ...
                                    foreach (List<string> servs in Servers.Values)
                                    {
                                        lock (servs)
                                        {
                                            if (servs.Contains(name))
                                            {
                                                ...
                                                Logger.LogWarn($"Player {name} is allready connected");
                        ...
                        lock (prelogin)
                            if (prelogin.ContainsKey(name))
                            {
                                ...
                                Logger.Log("Prelogin TimeOut of " + name);
                            }
                    }
                    catch (Exception e)
                    {
                        Logger.LogError($"Error in prelogin of: {name}\n{e}");
                    }
                ...
                case 7://list
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Logger.LogWarn("Server: " + name + " Doesn't exist (player list request)");
                    ...
                    {
                        Logger.LogError($"Error in list of: {name}\n{e}");
                    }
                ...
                case 11://suprime le fichier
                    ...
                    Logger.Log("Delete request of " + name + ".dat");
                ...
                rst3:
                    ...
                    {
                        Logger.LogError($"Error in delete request of {name}\n{e}");
                    }
                ...
                default:
                    ...
                    Logger.LogError($"Invalid Packet ({packet.Length} bytes) " + BitConverter.ToString(packet));
            ...
            if ((DateTime.Now - start).TotalMilliseconds > 1 && ((int)(DateTime.Now - start).TotalMilliseconds) != 30000)
                Logger.Log($"EndedRequest: Total: {(int)((DateTime.Now - start).TotalMilliseconds)}ms Rcv: {(int)((afterRcv - start).TotalMilliseconds)}ms Process+Send: {(int)((DateTime.Now - afterRcv).TotalMilliseconds)}ms {packetid} {name}");
        }
        catch (Exception e)
        {
